...
export class AuthorizationService {
  ...
  public authorizeOnBridge() {
    ...
    this.http.post(`https://${this.bridgeIp}/api`, body).subscribe(resp => {
      const respConverted = resp as { error?: { address: string; description: string; type: number }, success?: { clientkey: string; username: string; } }[];
      ...
      localStorage.setItem('hueApiKey', respConverted[0].success?.username ?? '');
    });
  }
}
